...
resource "aws_lb_listener" "authenticate" {
  ...
  load_balancer_arn = aws_lb.application.arn
  port              = "8082"
  # SaC Testing - Set protocol to http
  protocol          = "HTTP"
  certificate_arn   = var.certificate_arn
  # SaC Testing - Severity: critical - Set ssl_policy to "" 
  ssl_policy = ""
...
resource "aws_lb_target_group" "application" {
  ...
  protocol                           = "HTTP"           # Must be configured
  ...
  # SaC Testing - Severity: High - Set availability_zone to ""
  availability_zone = ""
  # Preferred value true
  ...
  health_check {
    ...
    interval          = 30
    # Preferred value HTTPS for application & TLS for network
    # SaC Testing - Severity: Critical - set protocl to http
    protocol            = "HTTP"
    timeout             = 5
...
resource "aws_lb_listener_rule" "forward" {
  ...
  action {
    ...
    authenticate_cognito {
      # SaC Testing - Severity: High - Set user_pool_arn to ""
      user_pool_arn = "tesing user_pool_arn in listener rule"
    }
...
resource "aws_lb_listener_rule" "redirect" {
  ...
  action {
    ...
    redirect {
      ...
      # SaC Testing - Severity: Critical - Set protocol to http
      protocol    = "HTTP for redirect" # Must be configured
